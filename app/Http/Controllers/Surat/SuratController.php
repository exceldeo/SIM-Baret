        ...
        try{
            if ($request->hasFile('surat')) {
                if ($request->file('surat')->isValid()) {
                    ...
                    $request->surat->storeAs('public', $filename);
                    $url = Storage::url($filename);
                    DB::update("UPDATE file_catatan set image_url = ?, waktu_upload = ?, validasi_oleh = ? WHERE catatan_id = ? AND jenis_surat = ?", 
                    [$url, date("Y-m-d H:i:s"), "Bambang", $request->catatan_id, $request->jenis_surat]);
                }
            }
